...
<body>
    ...
    <form action="equipment_input_office.php" method="POST">
        <label for="location_id">Location ID:</label>
        <input type="text" name="location_id" id="location_id" required><br>

        <label for="equipment_type">Equipment Type:</label>
		<input type="hidden" name="equipment_type" value="Office"> 
		<span>Office</span><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                header("Location: equipment_input_office.php?error=Equipment+already+exists");
                exit();
            }
			...
			header("location: equipment_input_office.php");
	...
	//Show Office Table Details when database table has been populated
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
